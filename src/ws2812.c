...
rgb_t cache_arr[WS2812_LED_COUNT];
...
void send_spi() {
    HAL_SPI_Transmit(&spi, ws2812_buffer, WS2812_BUFFER_SIZE, HAL_MAX_DELAY);
}
...
void rgb_update(u8 brightness) {
    ...
    if (brightness > 1) {
        ...
        for (i = 0; i < WS2812_LED_COUNT; i++) {
            ...
            ws2812_pixel(i, ((uint16_t)rgb->r * brightness) / 255,
                         ((uint16_t)rgb->g * brightness) / 255,
                         ((uint16_t)rgb->b * brightness) / 255);
        }
    }
    send_spi();
}
